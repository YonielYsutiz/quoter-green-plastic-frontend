<template>
 <el-menu
    ...
    >
      ...
      <el-menu-item src="http://localhost:5173/quoter/orderlist" index="2">Ordenes</el-menu-item>
  </el-menu>

  <div>
    <el-row :gutter="20">
      <el-col :span="24">
        <el-card class="centered-table" style="width: 100%;">
          <template #header>
            <h3><strong>Lista de ordenes</strong></h3>
          </template>
          <el-table :data="ordersList" border> 
            ...
            <el-table-column label="Descargar PDF" align="center" width="150">
            <template #default="{ row }">
              <div style="display: flex; justify-content: center; align-items: center; height: 100%;">
                <el-button type="success" style="min-width: 110px; height: 36px;" @click="downloadPdf(row.id)">
                  <el-icon><Download /></el-icon> Descargar PDF
                </el-button>
              </div>
            </template>
          </el-table-column>
...
const fetchOrdersList = async () => {
  try {
    const response = await axios.get('http://127.0.0.1:8000/api/quoters'); // Cambia la URL según tu API
    ordersList.value = response.data; // Asigna la respuesta a la variable ordersList
  } catch (error) {
...
const downloadPdf = async (id: number) => {
  try {
    const response = await axios.get(
      `http://127.0.0.1:8000/api/quoters/${id}/download`,
      {
        responseType: 'blob',
        headers: {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.data instanceof Blob) {
        try {
          ...
          errorDetails = errorText || 'Error en el servidor (500)';
        } catch {
          errorDetails = `Error ${error.response?.status}: ${error.response?.statusText}`;
        }
      } else {
    ...
    console.error('Error al descargar PDF:', errorDetails);
    throw new Error(`No se pudo descargar el PDF: ${errorDetails}`);
  }
};
